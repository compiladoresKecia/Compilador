...
public class Ambiente {
    ...
    public static void MostrarTabelaSimbolos() {
        ...
        System.out.println("Offset Identificador\tTipo\t\n");
        Set<Word> words = gerarHashMap();
        Iterator<Word> iterator = words.iterator();
        while (iterator.hasNext()) {
            Word palavra = (Word) iterator.next();
            if (palavra.getTag() == 294) {
                System.out.println(table.get(palavra).getOffset() + ":\t" + palavra.toString() + "\t\t" + palavra.getType() + "\t");
    ...
    /**
     * Carrega o offset do id.
     *
     * @return
     */
    public static Identificador gerarIdentificador() {
        int i = -1;
        Set<Word> words = gerarHashMap();
        Iterator<Word> iterator = words.iterator();
        while (iterator.hasNext()) {
